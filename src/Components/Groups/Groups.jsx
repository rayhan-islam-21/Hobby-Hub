...
const Groups = ({ group, deleteGroup }) => {
  ...
  const handleDelete = (id) => {
    fetch(`http://localhost:2000/allgroups/${id}`, {
      method: "DELETE",
    })
      .then((res) => {
        if (!res.ok) {
  ...
  return (
    <div className="bg-white dark:bg-white/10 dark:backdrop-blur-md border border-gray-300 dark:border-white/20 shadow-xl rounded-xl overflow-hidden max-w-md w-full mx-auto transition-transform duration-300 hover:scale-105 hover:shadow-2xl">
      ...
      <div className="p-6 flex flex-col justify-between h-64">
        ...
        <div className="flex justify-between mt-5">
          <button className="bg-gradient-to-r from-green-400 to-blue-500 text-white px-5 py-2 rounded-full font-semibold shadow-md hover:shadow-lg cursor-pointer transition">
            Details
          </button>
          <button
